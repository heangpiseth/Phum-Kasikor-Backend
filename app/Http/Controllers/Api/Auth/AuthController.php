...
class AuthController extends Controller
{
    public function register(Request $request)
    {
        ...
        if ($verification->type === 'phone') {
            $phoneInput = $request->identifier ?? $request->phone;
            $formattedPhone = preg_replace('/^0/', '885', $phoneInput);

            \Illuminate\Support\Facades\Http::withToken(env('MOCEAN_API_TOKEN'))
                ->asForm()
                ->post('https://rest.moceanapi.com/rest/2/sms', [
                    'mocean-from' => 'MOCEAN',
                    'mocean-to' => $formattedPhone,
                    'mocean-text' => "Your Phum Kasikor verification code is {$verification->code}"
                ]);
        }
